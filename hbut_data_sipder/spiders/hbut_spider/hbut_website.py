...
from hbut_data_sipder.spiders.hbut_spider.teacher_info_spider import Teacher
...
class Student:
    ...
    def getGrade(self, semester):
        ...
        # semesterNameStr = self.semesterNameDict[semester]
    ...
    def get_user_base_info(self):
        soup = BeautifulSoup(self.session.get("http://run.hbut.edu.cn/T_Student/OwnInfo").text, "html.parser")
        info_tr_line = soup.find_all("tr")[1]
        td_tags = info_tr_line.find_all("td")
        reply = {
            "class": td_tags[0].text,
            "student_id": td_tags[1].text,
            "name": td_tags[2].text,
            "id": td_tags[3].text,
            "gendar": td_tags[4].text,
            "nationality": td_tags[5].text,
            "colleage": td_tags[6].text,
            "subject": td_tags[7].text,
            "school_system": td_tags[8].text
        }
        return reply
...
if __name__ == '__main__':
    st = Student("1710221405", "100019")
    print(st.get_user_base_info())
