...
def paeser(request):
    ...
    """
    group,action = request.form.get("group"),request.form.get("action")
    return handle_for_base(group,action,request)

def handle_for_base(group,action,request_object):
    """
    路由函数
    :param group: 用户组
    ...
    """
    if group == "base":
        return uuia.get_response_data(action,request_object)
    else:
